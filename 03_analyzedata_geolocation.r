...
loc <- geocode("London",output="more")
lon<-loc$lon
lat<-loc$lat

#Plot coordinates on a Map
map <- get_map(location = c(lon = lon, lat = lat), zoom = 10,
                      maptype = "roadmap", scale = 2)

ggmap(map) +
        geom_point(data = df3, aes(x = x, y = y, fill = "red", alpha = 0), size = 4, shape = 21) +
        guides(fill=FALSE, alpha=FALSE, size=FALSE)

#CityWise & countrywise Reviewers
country_reviews<-table(df$country)
city_reviews<-table(df$city)
